						...
						<div class="row">
							<div class="col-xs-12">
								<div class="card">
									<div class="card-header">
										<div class="card-title">
											...
											<ul class="list-inline" id="select_one">
											  <li>编程语言：</li>
											  <li><a href="${pageContext.request.contextPath}/question/querysubject/.do" class="btn btn-sm  select-all ${empty questionsubject ?'btn-warning' : '' }">全部</a></li>
											  <li><a href="${pageContext.request.contextPath}/question/querysubject/1.do" class="btn btn-sm ${ questionsubject eq 1 ?'btn-warning' : '' }">java</a></li>
											  <li><a href="${pageContext.request.contextPath}/question/querysubject/2.do" class="btn btn-sm ${ questionsubject eq 2 ?'btn-warning' : '' }">C/C++</a></li>
											  <li><a href="${pageContext.request.contextPath}/question/querysubject/3.do" class="btn btn-sm ${ questionsubject eq 3 ?'btn-warning' : '' }">JavaScript</a></li>
											  <li><a href="${pageContext.request.contextPath}/question/querysubject/4.do" class="btn btn-sm ${ questionsubject eq 4 ?'btn-warning' : '' }">HTML</a></li>
											  <li><a href="${pageContext.request.contextPath}/question/querysubject/5.do" class="btn btn-sm ${ questionsubject eq 5 ?'btn-warning' : '' }">C#</a></li>
											</ul>
											
											<ul class="list-inline" id="select_two">
											  <li>题型：</li>
											  <li><a href="${pageContext.request.contextPath}/question/querytype/.do" class="btn btn-sm  select-all ${empty questiontype ?'btn-warning' : '' }">全部</a></li>
											  <li><a href="${pageContext.request.contextPath}/question/querytype/1.do" class="btn btn-sm ${ questiontype eq 1 ?'btn-warning' : '' }">单选题</a></li>
											  <li><a href="${pageContext.request.contextPath}/question/querytype/3.do" class="btn btn-sm ${ questiontype eq 3 ?'btn-warning' : '' }">判断题</a></li>
											  <li><a href="${pageContext.request.contextPath}/question/querytype/2.do" class="btn btn-sm ${ questiontype eq 2 ?'btn-warning' : '' }">多选题</a></li>
											  <li><a href="${pageContext.request.contextPath}/question/querytype/4.do" class="btn btn-sm ${ questiontype eq 4 ?'btn-warning' : '' }">填空题</a></li>
											  <li><a href="${pageContext.request.contextPath}/question/querytype/5.do" class="btn btn-sm ${ questiontype eq 5 ?'btn-warning' : '' }">简答题</a></li>
											</ul>
									...
									<form class="form-control-static form-inline form-wrapper">
											<div class="input-group">
								                 <input type="text" class="form-control questcont" value="${questioncontent}"/>
								                 <span class="input-group-addon"><a href="${pageContext.request.contextPath}/question/page/1/5.do" class="glyphicon glyphicon-search"></a></span>
								             </div>
									</form>
										
									<c:forEach items="${pageInfo.getList()}" var="quest">
										<div class="panel panel-default">
											<div class="panel-heading">
												<h4 class="panel-title">
													<a data-toggle="collapse" data-parent="#accordion" href="#collapse${quest.id}">
														<div>
														
															 <ul class="list-group">
															 <c:if test="${!empty quest.content }">
					                                            <li class="list-group-item">
					                                            	<p>${quest.content}</p>
					                                            </li>
					                                           </c:if>
					                                           <c:if test="${quest.questionType eq 1 or  quest.questionType eq 2}">
					                                            <c:if test="${!empty quest.option1 }">
						                                            <li class="list-group-item">
						                                            	<label>A：</label>
						                                            	${quest.option1}
						                                            </li>
					                                           ...
					                                           <c:if test="${!empty quest.option2 }">
					                                            <li class="list-group-item">
					                                            	<label>B：</label>
					                                            	${quest.option2}
					                                            </li>
					                                            </c:if>
					                                            <c:if test="${!empty quest.option3 }">
					                                             <li class="list-group-item">
					                                            	<label>C：</label>
					                                            	${quest.option3}
					                                            </li>
					                                            </c:if>
					                                            <c:if test="${!empty quest.option4}">
					                                             <li class="list-group-item">
					                                            	<label>D：</label>
					                                            	${quest.option4}
					                                            </li>
												...
												<div class="row">
												<a id="quest${quest.id}" class="btn btn-sm btn-info add_collect_question ${empty collectQuestions[quest.id] ?'':'hidden'}"  style="margin-left: 30px" ><i class="fa fa-heart"></i></i>收藏试题</a>
												<a id="quest${quest.id}" class="btn btn-sm btn-danger remove_collect_question ${empty collectQuestions[quest.id] ?'hidden':''}" style="margin-left: 30px"><i class="fa fa-heart-o"></i>取消收藏</a>
												
												
												<c:if test="${empty paper.questions[quest.id] }">
													<a class="btn btn-sm btn-success pull-right add_question" id="quest${quest.id}" >加入试卷</a>
													<a class="btn btn-sm btn-danger pull-right remove_question hidden" id="quest${quest.id}" >移出试卷</a>
												</c:if>
												<c:if test="${!empty paper.questions[quest.id] }">
													<a class="btn btn-sm btn-success pull-right add_question hidden" id="quest${quest.id}" >加入试卷</a>
													<a class="btn btn-sm btn-danger pull-right remove_question " id="quest${quest.id}" >移出试卷</a>
												</c:if>
												
												
												</div>
											</div>
											<div id="collapse${quest.id}" class="panel-collapse collapse">
												<div class="panel-body">
													<div >
															 <ul class="list-group">
					                                            <li class="list-group-item">
										...
										<div class="row pull-right">
													...
													<ul class="pagination pagination-lg">
														<li><a
															href="${pageContext.request.contextPath}/question/page/1/${pageInfo.pageSize}.do">
																<span aria-hidden="true">首页</span>
														</a></li>
														<li><a
															href="${pageContext.request.contextPath}/question/page/${pageInfo.getPrePage()}/${pageInfo.pageSize}.do"
															aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
														</a></li>
														<c:forEach begin="${Math.floor(pageInfo.getPrePage()/10)*10+1}"
															end="${Math.floor(pageInfo.getPrePage()/10)*10+10}" var="i">
															<c:if test="${i <=pageInfo.getPages()}">
												
																<c:if test="${i == pageInfo.getPageNum()}">
																	<li><a class="label"
																		href="${pageContext.request.contextPath}/question/page/${i}/${pageInfo.pageSize}.do">${i}</a></li>
																</c:if>
																<c:if test="${i != pageInfo.getPageNum()}">
																	<li><a
																		href="${pageContext.request.contextPath}/question/page/${i}/${pageInfo.pageSize}.do">${i}</a></li>
														...
														<li><a
															href="${pageContext.request.contextPath}/question/page/${pageInfo.getNextPage()}/${pageInfo.pageSize}.do"
															aria-label="Next"> <span aria-hidden="true">&raquo;</span>
														</a></li>
														<li><a
															href="${pageContext.request.contextPath}/question/page/${pageInfo.getPages()}/${pageInfo.pageSize}.do">
																<span aria-hidden="true">尾页</span>
														</a></li>
														<li><span><input style="width: 30px; height: 20px;"
																type="text" value="${pageInfo.getPageNum()}" class="pageno">/${pageInfo.getPages()}</span></li>
														<li><a href="#" id="jump"><span>跳转</span></a></li>
														<input type="hidden" id="pagesize" value="${pageInfo.pageSize}" />
														<input type="hidden" id="path"
															value="${pageContext.request.contextPath}" />
														<input type="hidden" id="pages" value="${pageInfo.getPages()}" />
													</ul>
											</div>
